...
export class MarcaController {
  ...
  async count(@param.where(Marca) where?: Where<Marca>): Promise<Count> {
  ...
  async find(@param.filter(Marca) filter?: Filter<Marca>): Promise<Marca[]> {
  ...
  async findById(
    ...
    @param.filter(Marca, {exclude: 'where'})
    filter?: FilterExcludingWhere<Marca>,
  ): Promise<Marca> {
